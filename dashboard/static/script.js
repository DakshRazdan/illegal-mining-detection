...
function switchIndex(name, btn) {
  ...
  if (btn.dataset) btn.dataset.index = name;
  currentIndex = name;
  ...
  const ps = document.getElementById('pipeline-status');
  if (ps) ps.textContent = `Loading ${name.toUpperCase()} overlay…`;

  // Priority 1: /api/temporal/frame/ — serves real Sentinel-2 data when
  // Colab bridge is loaded, or cached .npz disk data
  fetch(`${API_BASE}/api/temporal/frame/${idx}`)
    ...
    .then(d => {
      ...
      const img = d[key];
      if (img) {
        overlayIndexImage(img);
        const src = d.source === 'colab_ingest' ? 'Colab spectral RF' : d.source === 'sentinel2' ? 'Sentinel-2 real' : 'cached';
        if (ps) ps.textContent = `${name.toUpperCase()} · ${d.label || ''} · ${src}`;
        return;
      }
      // Priority 2: /api/mining/overlay/ synthetic fallback
      fetch(`${API_BASE}/api/mining/overlay/${currentAoi}/${name}?period_idx=${idx}`)
        .then(r => r.json())
        .then(data => {
          if (data.png_b64) {
            overlayIndexImage(data.png_b64);
            if (ps) ps.textContent = `${name.toUpperCase()} · OSM-derived · ${AOI_LABELS[currentAoi]}`;
          } else {
            if (ps) ps.textContent = `No ${name.toUpperCase()} data available`;
          }
        }).catch(() => { if (ps) ps.textContent = `${name.toUpperCase()} overlay unavailable`; });
    }).catch(() => {
      // Direct fallback
      fetch(`${API_BASE}/api/mining/overlay/${currentAoi}/${name}?period_idx=${idx}`)
        .then(r => r.json())
        .then(data => { if (data.png_b64) overlayIndexImage(data.png_b64); })
        .catch(() => { });
    });
}
...
function drawMiningActivity() {
  ...
  // Use real temporal mining scores if available from API, else fallback
  let data = window._realMine && window._realMine.length > 1 ? window._realMine : null;

  if (!data) {
    // Fallback: derive from real NDVI decline if available
    data = window._realNdvi && window._realNdvi.length > 1
      ? window._realNdvi.map(v => Math.max(0, (0.65 - v) * 2.2))  // invert NDVI → mining proxy
      : { jharkhand: [0.08, 0.10, 0.12, 0.14, 0.18, 0.22, 0.27, 0.31, 0.34, 0.38, 0.42, 0.46], odisha: [0.07, 0.09, 0.11, 0.13, 0.16, 0.20, 0.24, 0.28, 0.31, 0.35, 0.39, 0.43], chhattisgarh: [0.06, 0.08, 0.10, 0.12, 0.15, 0.19, 0.23, 0.27, 0.30, 0.34, 0.38, 0.42] }[currentAoi];
  }

  const mx = Math.max(...data), mn = Math.min(...data), rng = mx - mn || 0.01;
  const pts = data.map((v, i) => [(i / (data.length - 1)) * W, H - ((v - mn) / rng) * (H - 10) - 5]);

  // Area fill
  ctx.beginPath(); ctx.moveTo(pts[0][0], H); pts.forEach(p => ctx.lineTo(p[0], p[1])); ctx.lineTo(pts[pts.length - 1][0], H); ctx.closePath();
  ctx.fillStyle = 'rgba(240,165,0,0.1)'; ctx.fill();

  // Line
  ctx.beginPath(); ctx.moveTo(pts[0][0], pts[0][1]); pts.forEach(p => ctx.lineTo(p[0], p[1]));
  ctx.strokeStyle = '#f0a500'; ctx.lineWidth = 1.5; ctx.stroke();

  // Dots — color by anomaly (above mean = red, below = green)
  const mean = data.reduce((a, b) => a + b, 0) / data.length;
  pts.forEach((p, i) => {
    ctx.beginPath(); ctx.arc(p[0], p[1], 3, 0, Math.PI * 2);
    ctx.fillStyle = data[i] > mean ? '#ff3b3b' : '#00ff88';
    ctx.fill();
  });

  // Threshold line at mean
  const meanY = H - ((mean - mn) / rng) * (H - 10) - 5;
  ctx.setLineDash([3, 3]); ctx.beginPath(); ctx.moveTo(0, meanY); ctx.lineTo(W, meanY);
  ctx.strokeStyle = 'rgba(255,59,59,0.4)'; ctx.lineWidth = 1; ctx.stroke(); ctx.setLineDash([]);
}
